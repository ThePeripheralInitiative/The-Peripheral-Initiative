...
const PI = (function () {
  ...
  async function load() {
    ...
    const queries = [
      sb.from('posts')
        .select('id, author_id, genre, title, body, is_removed, created_at, profiles(username)')
        .eq('is_removed', false)
      ...
      sb.from('comments')
        .select('id, post_id, author_id, body, is_removed, created_at, profiles(username)')
        .eq('is_removed', false)
  ...
  async function addPost(genre, title, body) {
    ...
    if (error) {
      console.error('[Peripheral] addPost:', error);
